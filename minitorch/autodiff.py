...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        derivatives = []
        output = cls.backward(ctx, d_output)
        counter = 0
        for i in inputs:
            if is_constant(i):
                counter += 1
                ...
                try:
                    derivatives.append(VariableWithDeriv(i, output[counter]))
                except TypeError:
                    derivatives.append(VariableWithDeriv(i, output))
                counter += 1
...
def backpropagate(final_variable_with_deriv):
    q1 = list()
    dev_dict = {}
    q1.insert(0, final_variable_with_deriv.variable)
    dev_dict[final_variable_with_deriv.variable.name] = final_variable_with_deriv.deriv

    while len(q1) > 0:
        var = q1.pop()
        dev = dev_dict[var.name]

        if is_leaf(var):
            var._add_deriv(dev)
            continue
        hist = var.history
        derivatives = hist.backprop_step(dev)

        counter = 0
        for v in hist.inputs:
            if is_constant(v):
                continue
            else:
                if v not in q1:
                    q1.insert(0, v)
                if v.name in dev_dict:
                    dev_dict[v.name] += derivatives[counter].deriv
                else:
                    dev_dict[v.name] = derivatives[counter].deriv
                counter += 1
